...
cmd(
  ...
  async (from, sock, conText) => {
    ...
    if (!cmdName) return reply("❌ Which command? Example: *.setcmd ping*");

    const PROTECTED = ["setcmd", "dltcmd", "getcmd"];
    if (PROTECTED.includes(cmdName)) return reply("❌ *You cannot bind this command to itself!*");

    if (!findCmd(cmdName)) return reply(`❌ *Command not found:* \`${cmdName}\``);
    ...
    if (!stickerMsg?.fileSha256) {
      return reply("❌ *Reply to a sticker* with `.setcmd <command>`");
    }
...
cmd(
  ...
  async (from, sock, conText) => {
    ...
    const cmdName = (q || "").trim().toLowerCase();
    if (!cmdName) return reply("❌ Which command? Example: *.dltcmd ping*");
