...
import { fakeSender } from '../lib/messageConfig.js';
...
export default {
    ...
    handler: async (sock, message, args, context) => {
        const { chatId, prefix: contextPrefix } = context;
        const prefix = contextPrefix || config.prefixes[0];
        ...
        try {
            ...
            let menuText = `╭━━━  ⟮  ${config.botName || '★彡 [ ℝ𝔼𝔸ℙ𝔼ℝ 𝕏𝕄𝔻 ] 彡★'} COMMANDS ⟯━━━━━━┈⊷\n`;
            menuText += `┃✵╭──────────────\n`;
            ...
            // Check if image exists for thumbnail
            ...
            if (menuText.length > 32000) {
                ...
                // Send as image with caption for better visual
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: menuText
                }, { quoted: fakeSender });
            } else {
                await sock.sendMessage(chatId, { text: menuText }, { quoted: fakeSender });
            }
