...
async function sendSuccessConfirmation(sock, from, msg, mediaType, config, participantCount) {
  try {
    ...
    const botName = config?.botName || 'ALI-MD';
...
> ℹ️ *Quotes:* Use quotes for spaces: \`${botPrefix}groupstatus Hello -t "ali-md"\`

> ℹ️ *Combo:* Flags can mix: \`${botPrefix}groupstatus -c "join" -e "🌷" -t "ali-md" -ai --s https://example.com\`
...
cmd(
  ...
  async (from, sock, conText) => {
    ...
    if (realQuoted && !type) {
      if (!isSilent) await reply('❌ Invalid media type!');
      return;
    }
    ...
    try {
      ...
      if (!isSilent) {
        try {
          await react('❌');
          await reply(`❌ Failed to send status: ${e.message}`);
        } catch { }
      }
    }
  }
  ...
  async (from, sock, conText) => {
    ...
    if (type === 'txt') {
      ...
      try {
        ...
        return reply('❌ Failed to download media for broadcast.');
      }

      if (!buffer || buffer.length === 0) {
        return reply('❌ Media is empty, broadcast cancelled.');
      }
    ...
    if (!groupIds.length) {
      ...
      return reply('❌ Bot is not in any groups, or the group list has not finished syncing yet. Try again in a few seconds (especially if the bot just restarted).');
    }
    ...
    try {
      ...
      return reply(`❌ Failed to prepare status content for broadcast: ${genErr.message}`);
    }
    ...
    try {
      for (let idx = 0; idx < groupIds.length; idx++) {
        ...
        try {
          ...
          if (debugMode && !isSilent) {
            try { await reply(`❌ [${idx + 1}/${groupIds.length}] ${groupLabel} — failed: ${reason}`); } catch {}
          }
        }
