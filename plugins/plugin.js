    ...
    async (from, sock, conText) => {
        ...
        if (GIST_URL_RE.test(input)) {
            ...
            try {
                ...
                if (results.failed.length) lines.push(`❌ Failed: ${results.failed.join(', ')}`);
                lines.push(`🔗 ${data.html_url}`);

                await react(results.installed.length ? "✅" : "❌");
        ...
        if (!info) {
            await react("❌");
            return reply(`❌ *${name}* is not installed on this bot.`);
        }
...
cmd({
    ...
    const installed = await getInstalledPlugins();
    const info = installed[name];
    if (!info) return reply(`❌ *${name}* not found`);
...
cmd(
    ...
    async (from, sock, conText) => {
        ...
        if (!rawInput) return reply(`❌ Provide plugin name(s).\nEg: ${botPrefix}deleteplugin ping.js,alive.js`);
...
cmd(
    ...
    async (from, sock, conText) => {
        ...
        const name = normalizeName(args[0]);
        if (!name) return reply(`❌ Provide a plugin name.\nEg: ${botPrefix}updateplugin ping.js`);

        const { ok, installed } = await assertIsTrackedPlugin(name);
        if (!ok) return reply(`❌ *${name}* is not an installed Gist plugin.`);

        const info = installed[name];
        if (!info?.raw_url) return reply(`❌ No Gist info found for *${name}*. Reinstall it: ${botPrefix}plugin <gist_url>`);
        ...
        try {
            ...
            if (backupContent !== null) {
                ...
                await react("❌");
                return reply(`❌ Update failed (broken code) — reverted to old version.`);
            }
            await react("❌");
            return reply(`❌ Update failed (broken code), no backup to revert to.`);
        }
    }
);
