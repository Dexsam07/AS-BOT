...
function createContext({ sock, message, body, commandName, args, adminStatus = {} }) {
...
async function dispatchMessage({ sock, message, registry, logger = console }) {
  ...
  const preliminary = createContext({ sock, message, body, commandName, args: tokens });
  ...
  const context = createContext({ sock, message, body, commandName, args: tokens, adminStatus: { isSenderAdmin: decision.senderAdmin, isBotAdmin: decision.botAdmin } });
